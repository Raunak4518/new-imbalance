...
DATASET_NAME   = "ISIC_2020"
DATASET_SLUG   = "jpeg-melanoma-256x256"
DATASET_HANDLE = "cdeotte/jpeg-melanoma-256x256"
IS_COMPETITION = False
DOWNLOAD_CMD   = """kaggle datasets download cdeotte/jpeg-melanoma-256x256"""
NUM_CLASSES    = 2
SEED           = 42
IMG_SIZE       = 224
...
def find_file_recursive(base_path, target):
    ...
    for root, dirs, files in os.walk(base_path):
        if target in files or target in dirs:
            return True
        if target == "ISIC_2020_Training_GroundTruth_v2.csv" and "train.csv" in files:
            return True
    return False
...
def download_dataset(dest_path):
    """Download ISIC_2020 directly from official source (no Kaggle API needed)."""
    ...
    if kaggle_path:
        ...
        if find_file_recursive(dest, REQUIRED_FILE):
            ...
            if success:
                ...
                print("Or click '+ Add Data' and search for: cdeotte/jpeg-melanoma-256x256")
                sys.exit(1)
...
for root, _, files in os.walk(DATASET_PATH):
    for f in files:
        if f in ["train.csv", "ISIC_2020_Training_GroundTruth_v2.csv"]:
            meta_csv = os.path.join(root, f)
            break

if not meta_csv:
    print("ERROR: Could not find train.csv or ISIC_2020_Training_GroundTruth_v2.csv")
    sys.exit(1)
