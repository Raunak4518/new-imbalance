...
DATASET_SLUG   = "isic-2019"
DATASET_HANDLE = "nodoubttome/isic-2019"
IS_COMPETITION = False
DOWNLOAD_CMD   = """kaggle datasets download nodoubttome/isic-2019"""
NUM_CLASSES    = 8
SEED           = 42
IMG_SIZE       = 224
...
def find_kaggle_dataset_path(slug):
    base = "/kaggle/input"
    if not os.path.exists(base): return None
    if os.path.exists(os.path.join(base, slug)): return os.path.join(base, slug)
    dirs = os.listdir(base)
    for d in dirs:
        if slug in d or d in slug: return os.path.join(base, d)
    if len(dirs) > 0: return os.path.join(base, dirs[0])
    return None

KAGGLE_INPUT_PATH = find_kaggle_dataset_path(DATASET_SLUG)

if not DATASET_PATH:
    if os.path.exists("/kaggle/input"):
        if KAGGLE_INPUT_PATH and os.path.exists(KAGGLE_INPUT_PATH):
            DATASET_PATH = KAGGLE_INPUT_PATH
            print(f"Detected Kaggle environment. Using attached dataset at {DATASET_PATH}")
        else:
            if "kaggle datasets" in DOWNLOAD_CMD or "kaggle competitions" in DOWNLOAD_CMD:
                print(f"Dataset not attached. Attempting to download {DATASET_HANDLE} automatically via kagglehub...")
                try:
                    import kagglehub
                    if IS_COMPETITION:
                        DATASET_PATH = kagglehub.competition_download(DATASET_HANDLE)
                    else:
                        DATASET_PATH = kagglehub.dataset_download(DATASET_HANDLE)
                    print(f"Successfully downloaded to kagglehub cache: {DATASET_PATH}")
                except Exception as e:
                    print(f"ERROR downloading via kagglehub: {e}")
                    print("Please ensure internet is turned on for this Kaggle notebook, or use the 'Add Data' button.")
                    sys.exit(1)
            else:
                print(f"ERROR: Dataset requires manual download or non-kaggle source. Please attach via 'Add Data'.")
                sys.exit(1)
    else:
        ...
        if len(os.listdir(DATASET_PATH)) == 0:
            ...
            if "Manual Download Required" in DOWNLOAD_CMD or "Credential Required" in DOWNLOAD_CMD:
                print("==========================================================")
                print("MANUAL DOWNLOAD REQUIRED!")
                print(f"Please download the dataset manually and place images in {DATASET_PATH}")
                print("==========================================================")
                sys.exit(1)
            else:
                cmd = DOWNLOAD_CMD
                if cmd.startswith("kaggle datasets download"):
                    cmd = f"{cmd} -p \"{DATASET_PATH}\" --unzip"
