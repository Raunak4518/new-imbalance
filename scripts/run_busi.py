...
DATASET_NAME   = "BUSI"
DATASET_SLUG   = "breast-ultrasound-images-dataset"
DATASET_HANDLE = "aryashah2k/breast-ultrasound-images-dataset"
IS_COMPETITION = False
DOWNLOAD_CMD   = """kaggle datasets download aryashah2k/breast-ultrasound-images-dataset"""
NUM_CLASSES    = 3
SEED           = 42
IMG_SIZE       = 224
...
if not DATASET_PATH:
    ...
    if kaggle_path:
        ...
        print("2. Search for: aryashah2k/breast-ultrasound-images-dataset")
...
def is_valid_image(path):
    ...
    if any(p == 'mask' or p == 'masks' or p.endswith('_mask') for p in parts):
        return False
    return name.endswith(('.png', '.jpg', '.jpeg', '.bmp'))

if len(class_dirs) >= 2:
    print(f"Found {len(class_dirs)} class subdirectories. Using ImageFolder with mask filtering.")
    full_dataset = datasets.ImageFolder(root=img_root, transform=base_transform, is_valid_file=is_valid_image)
