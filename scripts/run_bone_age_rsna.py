...
DATASET_NAME   = "Bone_Age_RSNA"
DATASET_SLUG   = "rsna-bone-age"
DATASET_HANDLE = "kmader/rsna-bone-age"
IS_COMPETITION = False
DOWNLOAD_CMD   = """kaggle datasets download kmader/rsna-bone-age"""
NUM_CLASSES    = 5
SEED           = 42
IMG_SIZE       = 224
...
if not DATASET_PATH:
    ...
    if kaggle_path:
        ...
        print("2. Search for: kmader/rsna-bone-age")
...
if len(class_dirs) >= 2:
    ...
    if "boneage" in df.columns:
        print("Continuous boneage target detected. Binning boneage into 5 discrete categories.")
        bins = [0, 48, 96, 144, 192, 240]
        CLASS_NAMES = ['0-48m', '48-96m', '96-144m', '144-192m', '192-240m']
        df['binned_label'] = pd.cut(df['boneage'], bins=bins, labels=CLASS_NAMES)
        label_col = 'binned_label'
